...
import { findLicense } from "utils/functions";
...
function Form(props) {
  ...
  const { setStep, step, setExist, setClient, clientList } = props;
  ...
  function handleSubmit() {
    if (
      ...
      setStep(step+1);
      const { encryptedLicense, encryptedDescription, encryptedDate, encryptedProofOfLicense, client } = licenseInfo;
      findLicense(client, encryptedLicense, encryptedDescription, encryptedDate, encryptedProofOfLicense, (error, result) => {
        if (error) {
