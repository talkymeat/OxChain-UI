...
import { confirmAge } from "utils/functions";
...
function Form(props) {
  ...
  const { setStep, step, setResult, setClient, value, clientList } = props;
  ...
  function handleSubmit() {
    if (encryptedAge.trim() && encryptedProofOfAge.trim() && client.trim()) {
      setStep(step + 1);
      const { encryptedAge, encryptedProofOfAge, client } = ageInfo;
      confirmAge(client, encryptedAge, encryptedProofOfAge, (error, result) => {
        if (error) {
          props.dispatch(
